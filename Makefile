.PHONY: network up down recreate force-recreate drift-check workflow-size health-check edge-auth-check versions assert-tokens deploy-scope probe-delivery probe-status db-up db-shell db-check db-init deploy-root \
