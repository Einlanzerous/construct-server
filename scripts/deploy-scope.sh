...
#
#   * One pin can cover several services, and four of the thirteen do: SWITCHYARD_TAG
#     covers THREE (backend, frontend and switchyard-mcp — SERV-99), and APERTURE_TAG,
#     CENTRIFUGE_TAG and INTERLOCK_TAG cover two each. A repo's images ship from one
#     release (versions.env), so those recreate together, which is correct — they are one
#     version — but only nine pins name exactly one container.
...
# gives about the same mapping: a hardcoded list goes stale the day another service is
